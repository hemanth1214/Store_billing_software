...
class BillApp:
    def __init__(self, root):
        ...
        self.root.title("Welcome to store")
        self.root.title("BY  YHCC")
        bg_color = "grey"
        title = Label(self.root, text="Welcome to store   By YHCC", font=('times new roman', 30, 'bold'), pady=2, bd=12,
                      bg="green", fg="Black", relief=GROOVE)
        title.pack(fill=X)
        title = Label(self.root, text="By YHCC", font=('times new roman', 30, 'bold'), pady=2, bd=12,
                      bg="green", fg="Black", relief=GROOVE)
        title.pack(fill=X)
        # ================variables=======================
        ...
        oil_lbl = Label(F3, text="cooking Oil", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        oil_lbl.grid(row=1, column=0, padx=10, pady=10, sticky='W')
        oil_txt = Entry(F3, width=10, textvariable=self.oil, font=('times new roman', 16, 'bold'), bd=5,
                             relief=GROOVE)

        oil_txt.grid(row=1, column=1, padx=10, pady=10)

        grainpowder_lbl = Label(F3, text="grain", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        grainpowder_lbl.grid(row=2, column=0, padx=10, pady=10, sticky='W')
        grainprowder_txt = Entry(F3, width=10, textvariable=self.grainpowder, font=('times new roman', 16, 'bold'), bd=5,
                          relief=GROOVE)
        grainprowder_txt.grid(row=2, column=1, padx=10, pady=10)

        masala_lbl = Label(F3, text="masala", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        masala_lbl.grid(row=3, column=0, padx=10, pady=10, sticky='W')
        masala_txt = Entry(F3, width=10, textvariable=self.masala, font=('times new roman', 16, 'bold'), bd=5,
                           relief=GROOVE)
        masala_txt.grid(row=3, column=1, padx=10, pady=10)
        ...
        curd_lbl= Label(F3, text="curd", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        curd_lbl.grid(row=5, column=0, padx=10, pady=10, sticky='W')
        curd_txt = Entry(F3, width=10, textvariable=self.curd, font=('times new roman', 16, 'bold'), bd=5,
                          relief=GROOVE)
        curd_txt.grid(row=5, column=1, padx=10, pady=10)

        # ===========ColdDrinks================================
        F4 = LabelFrame(self.root, text="Drinks", font=('times new roman', 15, 'bold'), bd=10, fg="Black",
                        bg="#badc57")
        F4.place(x=670, y=180, width=325, height=380)

        coca_lbl = Label(F4, text="coca", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        coca_lbl.grid(row=0, column=0, padx=10, pady=10, sticky='W')
        coca_txt = Entry(F4, width=10, textvariable=self.coca, font=('times new roman', 16, 'bold'), bd=5,
                           relief=GROOVE)
        coca_txt.grid(row=0, column=1, padx=10, pady=10)
        ...
        fruitjucice_lbl = Label(F4, text="fruitucice", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        fruitjucice_lbl.grid(row=3, column=0, padx=10, pady=10, sticky='W')
        fruitjucice_txt = Entry(F4, width=10, textvariable=self.fruitjucice, font=('times new roman', 16, 'bold'), bd=5,
                         relief=GROOVE)
        fruitjucice_txt.grid(row=3, column=1, padx=10, pady=10)
        ...
        tea_lbl = Label(F4, text="Tea", font=('times new roman', 16, 'bold'), bg="#badc57",
                                 fg="black")
        tea_lbl.grid(row=5, column=0, padx=10, pady=10, sticky='W')
        tea_txt = Entry(F4, width=10, textvariable=self.tea, font=('times new roman', 16, 'bold'),
                                 bd=5, relief=GROOVE)
    ...
    def total(self):
        ...
        self.g_f_o_p = self.oil.get() * 10
        self.g_w_p = self.grainpowder.get() * 10
        self.g_s_p = self.masala.get() * 6
        self.g_f_p = self.flour.get() * 8
        self.g_m_p = self.curd.get() * 5
    ...
    def bill_area(self):
        ...
        if self.oil.get() != 0:
            self.txtarea.insert(END, f"\n cooking Oil\t\t{self.oil.get()}\t\t{self.g_f_o_p}")
        if self.grainpowder.get() != 0:
            self.txtarea.insert(END, f"\n grain\t\t{self.grainpowder.get()}\t\t{self.g_w_p}")
        if self.masala.get() != 0:
            self.txtarea.insert(END, f"\n masala\t\t{self.masala.get()}\t\t{self.g_s_p}")
        if self.flour.get() != 0:
            self.txtarea.insert(END, f"\n Flour\t\t{self.flour.get()}\t\t{self.g_f_p}")
        if self.curd.get() != 0:
            self.txtarea.insert(END, f"\n curd\t\t{self.curd.get()}\t\t{self.g_m_p}")
        # ================ColdDrinks==========================
        if self.coca.get() != 0:
            self.txtarea.insert(END, f"\n  coca\t\t{self.coca.get()}\t\t{self.c_d_s_p}")
        if self.mineral.get() != 0:
            self.txtarea.insert(END, f"\n Mineral\t\t{self.mineral.get()}\t\t{self.c_d_w_p}")
        if self.juice.get() != 0:
            self.txtarea.insert(END, f"\n Juice\t\t{self.juice.get()}\t\t{self.c_d_j_p}")
        if self.fruitjucice.get() != 0:
            self.txtarea.insert(END, f"\n fruitjucie\t\t{self.fruitjucice.get()}\t\t{self.c_d_c_p}")
        if self.lassi.get() != 0:
            self.txtarea.insert(END, f"\n Lassi\t\t{self.cream.get()}\t\t{self.c_d_l_p}")
        if self.tea.get() != 0:
            self.txtarea.insert(END, f"\n tea\t\t{self.sanitizer.get()}\t\t{self.c_m_d}")
            self.txtarea.insert(END, f"\n--------------------------------")
        # ===============taxes==============================
